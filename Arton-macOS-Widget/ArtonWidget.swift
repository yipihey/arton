...
struct ArtonWidgetEntryView: View {
        ...
        GeometryReader { geometry in
            ZStack {
                ...
                if let imagePath = entry.imagePath,
                   let nsImage = loadImage(from: imagePath, size: geometry.size) {
    ...
    private func loadImage(from path: String, size: CGSize) -> NSImage? {
        let url = URL(fileURLWithPath: path)

        // Generate thumbnail for performance
        let maxDimension = max(size.width, size.height) * (NSScreen.main?.backingScaleFactor ?? 2.0)
